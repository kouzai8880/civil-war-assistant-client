          ...
          if (roomUsers.size === 0) {
            ...
            try {
              ...
              Room.findByIdAndDelete(roomId).then(() => {
                console.log(`房间 ${roomId} 没有用户了，已从数据库中删除`);
              }).catch(err => {
                console.error(`从数据库中删除房间 ${roomId} 失败:`, err);
              });
            } catch (error) {
              console.error(`删除空房间时出错:`, error);
            }
          }
        }
          ...
          if (rooms.has(roomId)) {
            ...
            if (roomUser && roomUser.voiceChannel !== 'none') {
              ...
              if (voiceSystemMessageResult.success) {
                io.to(roomId).emit('new_message', voiceSystemMessageResult.message);
              }
            }

            // 如果房间为空，只从内存中删除房间，不从数据库删除
            if (room.size === 0) {
              rooms.delete(roomId);
              console.log(`房间 ${roomId} 没有在线用户了，从内存中删除`);
            } else {
              // 通知房间内其他用户该用户已离开
              ...
              if (systemMessageResult.success) {
                ...
                io.to(roomId).emit('new_message', systemMessageResult.message);
              }
            }
          }
        }
